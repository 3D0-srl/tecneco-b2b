...
class Export2Controller extends ApiController{
	...
	function display(){
		...
		$carrelli = $database->select('*','cart',"id= 12353");
		...
		foreach($carrelli as $carrello){
			...
			$ordini = $database->select('*','cartRow',"cart={$carrello['id']}");
			if( okArray($ordini) ){
	...
	function crea_dettaglio($ordine,$ordine_omaggio=false){
		...
		if( !$ordine_omaggio && $omaggio > 0){
			...
			$toreturn .= "\n".$this->crea_dettaglio($ordine,true);
		}
