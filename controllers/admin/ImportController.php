...
class ImportController extends AdminModuleController{
	...
	function import_clienti(){
		...
		$this->scrivi_log("Inizio importazione clienti");
		...
		$old_clienti = $database->select('codice_gestionale,id_user','b2b_cliente');
		...
		foreach($clienti as $v){
			...
			if( $v['ragsoc'] ){
				...
				$toinsert_user = array(
					...
					'password' =>password_hash($array['cf_piva'], PASSWORD_DEFAULT),
					'company' => $v['ragsoc'],
					'city' => $v['citta'],
				...
				if( !$old[$v['codice_gestionale']] ){
					...
					$this->scrivi_log("Inserito ".$v['codice_gestionale']);
				}else{
					//debugga('update');exit;
					...
					$database->update('b2b_cliente',"id_user={$id_user}",$toinsert_cliente);
					$this->scrivi_log("Aggiornato ".$v['codice_gestionale']);
		...
		foreach($destinazioni as $k => $v){
			if( $k >= 1  ){
				$data = array(
					'id_user' => $map_cli[$v['codice_cliente']],
					'name' => $v['descrizione'],
		...
		$this->scrivi_log("Fine importazione clienti");
		debugga('finito');exit;
	...
	function leggi_tracciato($tipo){
		...
		switch($tipo){
			...
			case 'articoli':
				...
				$file_destination = $GLOBALS['path_sincro']."GIACENZE/".$filename;
			...
			case 'ordini':
				...
				$filename = date('Y-m-d_H_i').".txt";
				$file_destination = $GLOBALS['path_sincro']."ORDINI_IMPORT/".$filename;
				if( file_exists($filepath_b2b) ){
