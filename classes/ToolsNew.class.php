...
class ToolsNew{
		...
		if( $data['quantity'] < $qnt ){
			...
			if( $qnt > 0 ){
				$response = array(
					...
					'back_order' => abs($qnt - $data['quantity'])
				);
			}else{
				$response = array(
					'success' => false,
					'error' => 'Il prodotto è esaurito',
					'max_qnt' => $data['quantity'],
					'back_order' => abs($qnt - $data['quantity'])
				);
			}
	...
	public static function buildPrice($id_user,$id_or_sku,$quantita=1,$nopromo=false){
		...
		$codice_listino_cliente2 = $data['codice_listino_cliente2'];
	...
	public static function getPrice(
		...
		$listini = self::getListini(
			...
			$data_user['codice_listino_cliente2'],
		...
		$sconto_aggiuntivo = 0;
		if( $campagna == 200 ){
			//pre la campagna 200 verifico se l'utente ha degli sconti
		...
		$prezzo_data = array(
			...
			'quantita_totale' => $quantita_totale?$quantita_totale:null
		);
	...
	public static function getListini(
		...
		$database = _obj('Database');
		//creazione delle condizione
		$codice_listino_cliente2 = trim($codice_listino_cliente2);
		$appoggio = array(
		...
		$now = date('Y-m-d');
		if( $nopromo ){
			$where .= "campagna <> '095' AND campagna <> '092' AND campagna <> '093' AND datainizio <= '{$now}' AND (";
		}else{
		...
		$where .= "(campagna='093' AND tipologia = 9 AND cliente='{$codice_listino_cliente2}' AND articolo='{$codice_articolo}') OR ";
		
		$where .= "(campagna='092' AND tipologia = 8 AND cliente='{$codice_listino_cliente}' AND articolo='{$codice_listino_cliente2}') OR ";
